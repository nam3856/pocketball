...
namespace LobbyRelaySample.UI
{
    ...
    public class RateLimitVisibility : MonoBehaviour
    {
        ...
        void Start()
        {
            MultiplayManager.Instance.LobbyManager.GetRateLimit(m_requestType).onCooldownChange += UpdateVisibility;
        }

        void OnDestroy()
        {
            if (MultiplayManager.Instance == null || MultiplayManager.Instance.LobbyManager == null)
                return;
            MultiplayManager.Instance.LobbyManager.GetRateLimit(m_requestType).onCooldownChange -= UpdateVisibility;
        }
